...
import copy
from knowledge_distillation import rollout, get_inner_model
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            if self.opts.multi_distribution_baseline:
                self.dataset = {
                    'uniform': self.problem.make_dataset(
                    size=self.opts.graph_size, num_samples=self.opts.val_size//3,
                    distribution= 'uniform', n_cluster=self.opts.n_cluster,
                    n_cluster_mix=self.opts.n_cluster_mix, mix_data=self.opts.generate_mix_data),
                    'cluster': self.problem.make_dataset(
                        size=self.opts.graph_size, num_samples=self.opts.val_size//3,
                        distribution='cluster', n_cluster=self.opts.n_cluster,
                        n_cluster_mix=self.opts.n_cluster_mix, mix_data=self.opts.generate_mix_data),
                    'mixed': self.problem.make_dataset(
                        size=self.opts.graph_size, num_samples=self.opts.val_size // 3,
                        distribution='mixed', n_cluster=self.opts.n_cluster,
                        n_cluster_mix=self.opts.n_cluster_mix, mix_data=self.opts.generate_mix_data),
                }
            else:
                self.dataset = self.problem.make_dataset(
                    size=self.opts.graph_size, num_samples=self.opts.val_size,
                    distribution=self.opts.data_distribution, n_cluster = self.opts.n_cluster,
                    n_cluster_mix= self.opts.n_cluster_mix, mix_data=self.opts.generate_mix_data)
        else:
