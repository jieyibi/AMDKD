...
class VRPDataset(Dataset):
    ...
    def __init__(self, filename=None, size=50, num_samples=1000000, offset=0,
                 distribution=None,n_cluster = 1, n_cluster_mix = 1, mix_data=False):
            ...
            if mix_data:
                ...
                for j in range(num_samples):
                    ...
                    if class_type.item() == 'uniform':
                        ...
                        coords = torch.zeros(size + 1, 2)
                        for i in range(n_cluster):
                            if i < n_cluster - 1:
                                coords[int((size + 1) / n_cluster) * i:int((size + 1) / n_cluster) * (i + 1)] = \
                                    torch.cat(
                                        (torch.FloatTensor(int((size + 1) / n_cluster), 1).normal_(mean_x[i], std),
                                         torch.FloatTensor(int((size + 1) / n_cluster), 1).normal_(mean_y[i], std)),
                                        dim=1)
                            elif i == n_cluster - 1:
                                coords[int((size + 1) / n_cluster) * i:] = \
                                    torch.cat((torch.FloatTensor((size + 1) - int((size + 1) / n_cluster) * i,1).normal_(mean_x[i], std),
                                               torch.FloatTensor((size + 1) - int((size + 1) / n_cluster) * i,1).normal_(mean_y[i], std)), dim=1)
                        ...
                        temp = {
                                'loc': coords[torch.arange(coords.size(0)) != depot_idx],
                                'demand': (torch.FloatTensor(size).uniform_(0, 9).int() + 1).float() / CAPACITIES[size],
                    ...
                    # with open('mix_data.csv', 'a+') as f:
                    #     import csv
                    #     csv_write = csv.writer(f)
                    #     dfrow = [str(class_type.item())]
            ...
            if distribution == 'uniform':
                ...
                for j in range(num_samples):
                    ...
                    for i in range(n_cluster):
                        if i < n_cluster - 1:
                            coords[int((size+1) / n_cluster) * i:int((size+1) / n_cluster) * (i + 1)] = \
                                torch.cat((torch.FloatTensor(int((size+1) / n_cluster),1).normal_(mean_x[i],std),
                                           torch.FloatTensor(int((size+1) / n_cluster),1).normal_(mean_y[i],std)),dim=1)
                        elif i == n_cluster - 1:
                            coords[int((size+1) / n_cluster) * i:] = \
                                torch.cat((torch.FloatTensor((size+1) - int((size+1) / n_cluster) * i, 1).normal_(mean_x[i], std),
                                           torch.FloatTensor((size+1) - int((size+1) / n_cluster) * i, 1).normal_(mean_y[i], std)), dim=1)
