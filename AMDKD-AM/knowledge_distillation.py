...
def train_epoch_distill(teacher_model, student_model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem,
    ...
    if opts.multi_teacher and opts.distill_distribution:
        ...
        if opts.adaptive_prob and epoch >=opts.start_adaptive_epoch:
            if opts.random_adaptive_prob!=0: # 选择用adaptive概率选teacher的概率（轮盘赌）
                if np.random.rand()<opts.random_adaptive_prob:# 随机选到了adaptive
    ...
    # time_enc_file = os.path.join(opts.log_dir, "{}_{}".format(opts.problem, opts.graph_size), opts.run_name,
    #                              'time', 'encoder_time.csv')
    # time_dec_file = os.path.join(opts.log_dir, "{}_{}".format(opts.problem, opts.graph_size), opts.run_name,
    #                              'time', 'decoder_time.csv')
    # import pandas as pd
    # enc = pd.read_csv(time_enc_file)
    # dec = pd.read_csv(time_dec_file)
    # enc = enc.sum()
    # dec = dec.sum()
    # if not opts.no_tensorboard:
    #     tb_logger.log_value('encoder_duration', enc, step)
    #     tb_logger.log_value('decoder_duration', dec, step)
    #     os.remove(time_enc_file)
    #     os.remove(time_dec_file)
...
def train_batch_distill(
    ...
    if opts.router == 'teacher':
        if opts.multi_teacher:
            ...
            for i in ['uniform','cluster','mixed']:
                ...
                # if opts.layer_loss:
                #     attn_file = os.path.join(opts.log_dir, "{}_{}".format(opts.problem, opts.graph_size), opts.run_name,'attn.pt')
                #     teacher_attn[i] = torch.load(attn_file)
                #     os.remove(attn_file)
            # Student
            ...
            # if opts.layer_loss:
            #     attn_file = os.path.join(opts.log_dir, "{}_{}".format(opts.problem, opts.graph_size), opts.run_name, 'attn.pt')
            #     student_attn = torch.load(attn_file)
            #     os.remove(attn_file)
            ...
            # if opts.layer_loss:
            #     attn_file = os.path.join(opts.log_dir, "{}_{}".format(opts.problem, opts.graph_size), opts.run_name, 'attn.pt')
            #     teacher_attn = torch.load(attn_file)
            #     os.remove(attn_file)
    ...
    # model_duration = time.time() - time_begin
    # print("Finished model, took {} s".format(model_duration))
    # tb_logger.log_value('model_duration', model_duration, step)
    ...
    if opts.multi_teacher:
        ...
        # loss = torch.log10(-task_loss + 1) + torch.log10(soft_loss + 1)
        loss = task_loss * (1 - opts.distill_alpha)  + soft_loss * opts.distill_alpha
        # loss = task_loss
    ...
    # bl_duration = time.time() - time_model_end
    # print("Finished baseline, took {} s".format(bl_duration))
    # Logging
