...
class TSPTrainer:
    def __init__(self,
        ...
        if self.trainer_params['logging']['tb_logger']:
            ...
            print('./log/' + get_start_time() + self.trainer_params['tb_path'])
        ...
        if trainer_params['distillation'] and trainer_params['distill_param']['multi_teacher']:# 3 teachers
            ...
            self.model =  {'uniform': model_uniform,
                            ...
                            'mixed': model_mixed}
            self.env = {'uniform': Env(**self.env_params),
                        'cluster': Env(**self.env_params),
                        'mixed': Env(**self.env_params)}
        else:
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            if is_test:
                ...
                for k, v in self.trainer_params['val_dataset_multi'].items():
                    ...
                    val_aug.append(aug)
                    gap_no_aug.append(
                        (no_aug - self.trainer_params['LKH3_optimal'][g]) / self.trainer_params['LKH3_optimal'][g])
                    gap_aug.append(
                        (aug - self.trainer_params['LKH3_optimal'][g]) / self.trainer_params['LKH3_optimal'][g])
                    g += 1
            ...
            if self.trainer_params['distillation'] and self.trainer_params['distill_param']['adaptive_prob']:
                ...
                if any(map(lambda x: x < 0, gap)):
                    ...
                    if self.trainer_params['distill_param']['adaptive_prob_type'] == 'softmax':
                        ...
                        self.trainer_params['distill_param']['teacher_prob'] = [gap[i] / sum(gap) for i in
                                                                                range(len(gap))]
            ...
            if epoch > 1:
                ...
                if self.trainer_params['distillation']:
                    ...
                    if self.trainer_params['distill_param']['distill_distribution']:
                        log_image_params = { 'json_foldername': 'log_image_style',
                                             'filename': 'style_tsp_{}_{}.json'.format(self.env_params['problem_size'],class_type)}
                ...
                if is_test:
                    ...
                    for i in range(3):
                        log_image_params = { 'json_foldername': 'log_image_style',
                                             'filename': 'style_tsp_{}_{}.json'.format(self.env_params['problem_size'],note[i])}
                ...
                if self.trainer_params['best']==0:
                    ...
                    if self.trainer_params['distillation'] and self.trainer_params['distill_param']['distill_distribution']:
                        torch.save(checkpoint_dict,
                                   '{}/checkpoint-{}-best-{}.pt'.format(self.result_folder,class_type,epoch))
                    else:
    ...
    def _train_one_batch(self, batch_size):
        ...
        ###############################################
        self.model.train()
        self.env.load_problems(batch_size, distribution=self.env_params['distribution'])
    ...
    def _distill_one_batch(self, batch_size,distribution=None):
        ...
        self.student_env.load_problems(batch_size, copy=problems)
        ...
        if distill_param['router'] == 'teacher':# Teacher as the router
            ...
            with torch.no_grad():
                ...
                teacher_probs = torch.zeros(size=(batch_size, self.env.pomo_size, self.env.problem_size, 0))
                # Decoding
                while not done:
            ...
            student_probs = torch.zeros(size=(batch_size, self.student_env.pomo_size, self.student_env.problem_size, 0))
            # Decoding
            selected_count = 0
            while not student_done:
                student_selected, student_prob, probs = self.student_model(student_state, route=teacher_pi, return_probs=True,selected_count=selected_count)
                # shape: (batch, pomo)
                selected_count += 1
            ...
            student_probs = torch.zeros(
                size=(batch_size, self.student_env.pomo_size, self.student_env.problem_size, 0))
            # Decoding
            while not student_done:
            ...
            with torch.no_grad():
                ...
                selected_count = 0
                while not done:
                    selected, prob, probs = self.model(state, route=student_pi, return_probs=True,selected_count=selected_count)
                    # shape: (batch, pomo)
                    selected_count += 1
                    state, reward, done = self.env.step(selected)
                    teacher_prob_list = torch.cat((teacher_prob_list, prob[:, :, None]), dim=2)
        ...
        if distill_param['meaningful_KLD']:
            ...
            soft_loss = nn.KLDivLoss()(student_probs.log(), teacher_probs) if not distill_param['KLD_student_to_teacher'] \
                ...
                # print(task_loss, attn_loss,soft_target_loss)
